...
stages{
	...
	stage('Pushing Image to DockerHub'){
		steps{
			sshPublisher(publishers: [sshPublisherDesc(configName: 'dev', transfers: [sshTransfer(cleanRemote: false, excludes: '', execCommand: 'echo $DOCKERHUB_PSW | docker login -u $DOCKERHUB_USR --password-stdin && docker tag webapp:latest pritidevops/webapp:latest && docker push pritidevops/webapp:latest', execTimeout: 120000, flatten: false, makeEmptyDirs: false, noDefaultExcludes: false, patternSeparator: '[, ]+', remoteDirectory: 'maven-project-demo', remoteDirectorySDF: false, removePrefix: '', sourceFiles: '**/*')], usePromotionTimestamp: false, useWorkspaceInPromotion: false, verbose: true)])  
		}
	}
